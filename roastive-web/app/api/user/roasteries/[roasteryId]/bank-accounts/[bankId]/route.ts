...
export const PATCH = wrapAuth(async ({ headers }, req, { params }) => {
  ...
  const bankId = resolveParam(params?.bankId)
  if (!roasteryId || !bankId) {
    return NextResponse.json({ message: 'roasteryId and bankId are required' }, { status: 400 })
  }
  const body = await readJson(req)
  const nextHeaders = withRoasteryHeader(headers, roasteryId)
  nextHeaders.set('Content-Type', 'application/json')
  const { res, data } = await fetchJson(`${API_BASE}/v2/roasteries/bank-accounts/${encodeURIComponent(bankId)}`, {
    method: 'PATCH',
    headers: nextHeaders,
