...
export const PATCH = wrapAuth(async ({ headers }, req, { params }) => {
  ...
  const addressId = resolveParam(params?.addressId)
  if (!roasteryId || !addressId) {
    return NextResponse.json({ message: 'roasteryId and addressId are required' }, { status: 400 })
  }
  const body = await readJson(req)
  const nextHeaders = withRoasteryHeader(headers, roasteryId)
  nextHeaders.set('Content-Type', 'application/json')
  const { res, data } = await fetchJson(`${API_BASE}/v2/roasteries/addresses/${encodeURIComponent(addressId)}`, {
    method: 'PATCH',
    headers: nextHeaders,
