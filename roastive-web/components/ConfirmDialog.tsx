...
export default function ConfirmDialog({ open, title = '확인', message = '이 작업을 계속하시겠습니까?', confirmText = '확인', cancelText = '취소', onClose, onConfirm }: Props) {
  return (
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-md rounded-lg bg-white p-6 shadow-lg ring-1 ring-black/5 dark:bg-gray-900 dark:text-white dark:ring-white/10">
          ...
          <div className="mt-6 flex justify-end gap-2">
            ...
            <button onClick={onConfirm} className="rounded-md bg-red-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-red-500">{confirmText}</button>
          </div>
        </DialogPanel>
      </div>
    </Dialog>
